...
function addNoti() {
    if (!document.getElementById("zawgyi_noti")) {
        ...
        var title_value = "ဤစာမျက်နှာတွင် ရှိသော ဇော်ဂျီဖြင့် ရေးထားသည့် စာများအား အလိုအလျောက် ပြောင်းလဲထားပါသည်။";
        notify_node.setAttribute("title", title_value);
        body.appendChild(notify_node);
        var noti_off = document.getElementById("mua_noti_off");
        noti_off.addEventListener("click",
            function removeNoti() {
                clearNoti();
                noti_off.removeEventListener("click", removeNoti, false);
            }, false);
...
function clearNoti() {
    var noti = document.getElementById('zawgyi_noti');
    if (!noti) {
        return;
    }
    var bodyNode = document.body;
    bodyNode.removeChild(noti);
...
if (!list) {
    if (document.addEventListener) {
        ...
        document.addEventListener("DOMContentLoaded",
            function callAddObserber() {
                //free resources
                document.removeEventListener("DOMContentLoaded", callAddObserber, false);
                console.log("resources free");
                //add observer
                addObserver();
            }, false);
    }
